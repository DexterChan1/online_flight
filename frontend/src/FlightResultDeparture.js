...
const FlightResultDeparture = () => {
  ...
  useEffect(() => {
    ...
    if (fromCity)
      params.append("departureAirportCity", location.state.fromCity);
    if (toCity) params.append("arrivalAirportCity", location.state.toCity);
    // if (departureDate)
  ...
  const selectFlight = (flight) => {
    ...
    if (
      selectedDepartureFlight.id === flight.flightId &&
      selectedDepartureFlight.fareType === flight.fareType
    ) {
      ...
      setSelectedDepartureFlight(newSelectedFlight);
  ...
  const isSelected = () => {
    return (
      selectedDepartureFlight.id !== null &&
      selectedDepartureFlight.fareType !== null
    );
  };
  ...
  const proceed = () => {
    if (isSelected()) {
      // If a flight is selected, proceed to the next step
      if (tripType === "round_trip") {
        navigate("/flights_result_returning", {
          state: { ...location.state, selectedDepartureFlight },
        });
      } else {
      ...
      <main className="flex-grow p-8 z-10">
        <div className="max-w-6xl mx-auto bg-white rounded-lg shadow py-4 px-6">
          <h2 className="text-2xl font-bold text-center mb-4">
            Departure Flights Search Result Shown:
          </h2>
                  ...
                  <tr key={flight.flightId} className="border-b">
                    ...
                    <td>
                      ...
                      <button
                        ...
                        className={`ml-2 ${
                          selectedDepartureFlight?.id === flight.flightId &&
                          selectedDepartureFlight?.fareType === "economy"
                            ? "bg-yellow-600"
                            : "bg-yellow-300"
                    ...
                    <td>
                      ...
                      <button
                        ...
                        className={`ml-2 ${
                          selectedDepartureFlight?.id === flight.flightId &&
                          selectedDepartureFlight?.fareType === "business"
                            ? "bg-yellow-600"
                            : "bg-yellow-300"
                        } hover:bg-yellow-400 text-black font-bold py-1 px-2 rounded`}
                      >
                    ...
                    <td>
                      ...
                      <button
                        ...
                        className={`ml-2 ${
                          selectedDepartureFlight?.id === flight.flightId &&
                          selectedDepartureFlight?.fareType === "firstClass"
                            ? "bg-yellow-600"
                            : "bg-yellow-300"
                        } hover:bg-yellow-400 text-black font-bold py-1 px-2 rounded`}
                      >
...
export default FlightResultDeparture;
